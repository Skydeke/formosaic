...
fn sdk_dir(workspace: &Path) -> PathBuf {
    workspace.join(".android/sdk")
}
fn ndk_home(workspace: &Path) -> PathBuf {
    sdk_dir(workspace).join("ndk").join(NDK_VERSION)
}
...
fn cargo_apk(workspace: &Path, apk_args: &[&str]) {
    ...
    cmd.env(
        "BINDGEN_EXTRA_CLANG_ARGS_armv7_linux_androideabi",
        &bindgen_armv7,
    );
...
fn setup_android(workspace: &Path) {
    ...
    if !tools.exists() {
        ...
        run(&[
            ...
            zip,
            "-d",
            sdk.join("cmdline-tools").to_str().unwrap(),
        ]);
...
fn print_check(label: &str, ok: bool, path: &str) {
    if ok {
        ...
        println!("  ❌  {label} not found — run: cargo setup-android");
    }
}
