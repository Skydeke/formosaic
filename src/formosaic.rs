...
impl Formosaic {
    ...
    fn trigger_rescramble(&mut self, ctx: &mut SceneContext) {
        if let GameState::Restoring { .. } = self.game_state { return; }
        let model = match &self.model { Some(m) => m.clone(), None => return };
        model.borrow().upload_lerp(0.0);

        let search = best_scramble_axis(
            &model.borrow(),
            ENTROPY_CANDIDATES, TARGET_WORLD_RADIUS, CAMERA_FOV
        );
        self.entropy_report = Some(search.report);
        ...
        let solution_dir = if let Some(entity) = &self.entity {
            (entity.borrow().transform().rotation * search.axis).normalize()
        } else { search.axis };
        ...
        if let Some(camera) = ctx.camera() {
            let target = self.orbit.as_ref().map(|o| o.target).unwrap_or_else(|| Vector3::new(0.0, 0.0, 0.0));
            let dist   = self.orbit.as_ref().map(|o| o.distance).unwrap_or(3.0);
            let (ctrl, sp) = make_scrambled_orbit(target, dist, solution_dir);
            camera.borrow_mut().transform.position = sp;
            camera.borrow_mut().set_controller(Some(Box::new(ctrl)));
        }
    }
...
impl Application for Formosaic {
    ...
    fn on_event(&mut self, event: &Event, ctx: &mut SceneContext) {
        match event {
            ...
            Event::KeyDown { key: Key::L } => self.trigger_rescramble(ctx),
