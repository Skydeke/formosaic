...
pub fn register(scene: &Scenegraph, state: Rc<RefCell<UiState>>) {
    let hud = UiNode::new("hud", move |ui, w, h, _ctx| {
        ...
        ui.window("##hud")
            ...
            .position([w - scale.su(210.0) - scale.pad_w(), scale.pad_w()], Condition::Always)
            .size([scale.su(210.0), scale.su(100.0)], Condition::Always)
            .build(|| {
                if let Some(diff) = s.difficulty {
                    util::text_body(ui, &format!("{:.1}s  |  {}", s.elapsed_secs, difficulty_label(diff)));
                }
                if s.hint_count > 0 {
                    let label = format!("{} {}", s.hint_count, if s.hint_count == 1 { "hint" } else { "hints" });
