...
impl Mesh {
    ...
    pub fn scramble_along_axis(&mut self, axis: Vector3<f32>, min_disp: f32, max_disp: f32) {
        ...
        // IMPORTANT: always displace in the *positive* axis direction only.
        // This is the key invariant of the puzzle: when the camera looks along
        // `axis` all triangles' displacements project to the same apparent
        // depth, so the model looks seamless.  Using ±sign would cause half the
        // triangles to move toward the viewer and half away, making the model
        // appear exploded even from the correct angle.
        let tri_count = n / 9;
        for tri in 0..tri_count {
